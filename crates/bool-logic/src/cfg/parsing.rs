...
use super::ast::{All, Any, Expr, Not, Pred, all, any, expr, not};
...
pub fn parse(mut s: &str) -> Result<'_, Expr> {
    let s = &mut s;
    skip_space0(s);
...
mod tests {
    ...
    fn cfg_string() {
        let input =
            r#"  all(not(any(target_os = "linux", target_os = "macos")), all(), any(unix))  "#;
