...
use bool_logic::cfg::ast::Expr as CfgExpr;
use bool_logic::cfg::ast::all as cfg_all;
    ...
    fn visit_file_mut(&mut self, ast: &mut File) {
        ...
        while i < ast.items.len() {
            if Self::matches(&ast.items[i]) {
                ...
                ast.items.remove(i);
                insert_vec(&mut ast.items, i, items);
            } else {
                i += 1;
            }
        }
...
fn insert_vec(vec: &mut Vec<Item>, index: usize, items: Vec<Item>) {
    vec.splice(index..index, items);
}
...
fn parse_cfg(tokens: &TokenStream) -> CfgExpr {
    ...
    let result = bool_logic::cfg::parsing::parse(&tokens);
    match result {
        Ok(expr) => expr,
        Err(e) => panic!(
