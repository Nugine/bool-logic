...
{
    match expr {
        Expr::Any(any) => v.visit_mut_any(any),
        Expr::All(all) => v.visit_mut_all(all),
        Expr::Not(not) => v.visit_mut_not(not),
        Expr::Var(var) => v.visit_mut_var(var),
        Expr::Const(b) => v.visit_mut_const(b),
    }
}
